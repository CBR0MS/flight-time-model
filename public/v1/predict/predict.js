...
(async () => {
  ...
  $.getJSON("../data/meta/lookup_all.json", function(json) {
    ...
    (async () => {
      ...
      let depart,
        arrive = makePrediction(inputList, departModel, arrivalModel);
      depart = depart || 0;
      arrive = arrive || 0;
      ...
      const timeAtOrigin =
        lookup["airport_codes_to_taxi_time"][usrDate.getMonth()][usrOrigin][
          "avg_out"
        ];
      let key = usrOrigin + "_" + usrDest;
      const timeInAir =
        lookup["airport_codes_to_air_time"][usrDate.getMonth()][key];
      const timeAtDest =
        lookup["airport_codes_to_taxi_time"][usrDate.getMonth()][usrDest][
          "avg_in"
        ];
      ...
      let total = Math.round(
        timeAtOrigin + depart + (timeInAir + arrive) + timeAtDest
      );
      ...
      chartContent["data"]["content"][0]["label"] =
        "At " + usrOrigin + " (" + timeConvert(timeAtOrigin + depart) + ")";
      chartContent["data"]["content"][1]["value"] = dest;
      chartContent["data"]["content"][1]["label"] =
        "At " + usrDest + " (" + timeConvert(timeAtDest) + ")";
      chartContent["data"]["content"][2]["value"] = air;
      chartContent["data"]["content"][2]["label"] =
        "In Air (" + timeConvert(timeInAir + arrive) + ")";
      chartContent["header"]["title"]["text"] = timeConvert(total);

      $(document).ready(function() {
        chartContent["size"]["canvasWidth"] = $(document).width() / 2;
        if ($(document).width() < 600) {
          chartContent["size"]["canvasWidth"] =
            $(document).width() - $(document).width() / 3;
        }
