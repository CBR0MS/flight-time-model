...
class PanelGroup extends React.Component {
        ...
        const newContent = content.map((value, index) => {
          if (this.props.fadeOut) {
            ...
            this[timeoutName] = setTimeout(() => {
              const newContent = this.state.messages.filter(
                value1 => value.content === value1.content
              );
  ...
  render() {
    if (
      ...
      this.props.values[0].val !== undefined &&
      this.props.values[0].val !==
        this.state.messages[this.props.values[0].index].content
    ) {
      this.updateMessages();
    }
    ...
    if (this.props.fadeOut) {
      loading = (
        ...
        >
          {props => <div style={Object.assign(props, styles.loadingBar)} />}
        </Spring>
      );
    }
    ...
    return (
      <div style={revisedContainerStyle}>
        <Transition
          ...
          keys={item => item.key}
          from={{
            marginTop: -1 * (this.props.badgeStyle.height + 40),
            opacity: 0
          }}
          leave={{
            marginTop: -1 * (this.props.badgeStyle.height + 40),
