...
class CheckInputForm extends React.Component {
  ...
  handleAirlineSelect(id, airline) {
    const reducedArr = this.props.autocompleteDataAirlines.filter(
      value => airline === value.label
    );
  ...
  componentDidMount() {
    ...
    const passedOrigin =
      this.props.params.origin !== undefined &&
      this.props.params.origin.length > 0
        ? this.findMostSimilarInput(this.props.params.origin, this.locations)
        : this.state.origin;
    const passedDest =
      this.props.params.dest !== undefined && this.props.params.dest.length > 0
        ? this.findMostSimilarInput(this.props.params.dest, this.locations)
        : this.state.destination;

    const passedAirlines =
      this.props.params.airlines !== undefined &&
      this.props.params.airlines.length > 0
    ...
    //                             : true
    const passedDate =
      this.props.params.date !== undefined
    ...
    if (this.props.params.error === "emptyPlaces") {
      alerts.push(
        "Be sure to include a valid origin and destination for your flight"
      );
    }
    ...
    if (this.props.params.error === "badConnection") {
      alerts.push(
        "Be sure to include a valid connection airport for your flight"
      );
    }
    ...
    if (!this.state.allAirlines) {
      ...
      const listItems = (
        <PanelGroup
          ...
          showValue={airline => {
            return this.props.autocompleteDataAirlines.filter(
              value => airline === value.value
            )[0].label;
          }}
      ...
      airlinePicker = (
        <div style={styles.allAirlineContentWrapper}>
          ...
          <div style={styles.airlinesSection}>{listItems}</div>
        </div>
      );
    }
            ...
            <div style={styles.checkStyle}>
              <div style={styles.panelWrapper}>
                ...
                <GoButton
                  ...
                  shadow={true}
                  link={
                    `/predict?origin=${this.state.org_id}` +
                ...
                >
                  <span style={{ color: styles.darkBlue }}>
