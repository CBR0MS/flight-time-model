...
export const FlexTable = props => {
  return <div style={styles.flexTable}>{props.children}</div>;
};
    ...
    <div
      style={
        props.style !== undefined
          ? Object.assign(props.style, styles.inlineWrapper)
          : styles.inlineWrapper
      }
    >
      <div
        style={
          props.floatNum !== undefined
            ? { float: props.floatNum, display: "inline-block" }
            : {}
        }
      >
        <h1
          style={Object.assign(
            { fontSize: "2.5rem" },
            styles.inlineWrapperNoMargin
          )}
        >
          {props.title}
        </h1>
        {props.suffix !== undefined ? (
          <p style={styles.inlineWrapperNoMargin}>{props.suffix}</p>
        ) : null}
      </div>
      <p>{props.caption}</p>
    </div>
