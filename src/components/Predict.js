...
import {
  getDataFromAPI,
  makePredictions,
  filterListOfAirlinesWithAirports,
  constructSidebar,
  constructMain
} from "./Helpers/Predict";
...
class Predict extends React.Component {
  ...
  setRedirectError(error) {
    this.setState({
      redirectLoc: `/check${this.props.location.search}&error=${error}`
    });
  }
      ...
      try {
        ...
        if (
          ...
          if (airlinesLists.completeAirlinesList.length === 0) {
            alerts.push(
              "No airlines you entered fly this route. Comparing all airlines instead."
            );
            params.allAirlines = true;
            airlinesLists = filterListOfAirlinesWithAirports(params, newData);
          } else {
            alerts.push(
              "Some airlines you selected do not fly the route you entered."
            );
          }
        ...
        const predictions = await makePredictions(
          newData,
          airlinesLists.completeAirlinesList,
          meta
        );
        ...
        switch (err.err) {
          ...
          case "tooManyRequests":
            ...
            this.setPageError(
              `You've made too many requests. Please try again in ${seconds} seconds.`,
              false
            );
